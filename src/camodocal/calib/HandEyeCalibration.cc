...
HandEyeCalibration::estimateHandEyeScrewInitial(Eigen::MatrixXd &T,
  ...
  {
    if (mVerbose) {
      std::cout << "# INFO: No unique solution, returned an arbitrary one. "
                << std::endl;
    }
  ...
  if (u1.dot(v1) != 0.0) {
    ...
    if (u1.norm() == 0 && u2.norm() > 0) {
      ...
      ss << "camodocal::HandEyeCalibration error: normalization could not be "
            "handled. Your rotations and translations are probably either not "
            "aligned or not passed in properly.";
      ss << "u1:" << std::endl;
      ss << u1 << std::endl;
      ss << "v1:" << std::endl;
      ss << v1 << std::endl;
      ...
      ss << "v2:" << std::endl;
      ss << v2 << std::endl;
      ss << "Not handled yet. Your rotations and translations are probably "
            "either not aligned or not passed in properly."
         << std::endl;
  ...
  Eigen::Quaterniond d(q_prime_coeffs(0), q_prime_coeffs(1), q_prime_coeffs(2),
                       q_prime_coeffs(3));
