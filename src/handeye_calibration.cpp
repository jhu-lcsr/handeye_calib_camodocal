...
Eigen::Affine3d estimateHandEye(const EigenAffineVector &baseToTip,
  ...
  for (int i = 0; i < baseToTip.size(); ++i, ++t1_it, ++t2_it) {
    ...
    if (firstTransform) {
      ...
      rvecsArm.push_back(
          eigenRotToEigenVector3dAngleAxis(robotTipinFirstTipBase.rotation()));
...
void addFrame() {
  ...
  if (listener->waitForTransform(cameraTFname, ARTagTFname, now,
    ...
    hasCam = false;
    ROS_WARN("Fail to Cam TF transform between %s to %s", cameraTFname.c_str(),
             ARTagTFname.c_str());
  }
  ...
  if (hasEE and hasCam) {
    ...
    if (firstTransform) {
      ...
      std::cerr << "EE rot: (" << EETransform.getRotation().getAxis().getX()
...
int main(int argc, char **argv) {
  ...
  while (ros::ok()) {
    ...
    if ((key == 's') || (key == 'S')) {
      ...
      Eigen::Transform<double, 3, Eigen::Affine> resultAffineInv =
          resultAffine.inverse();
      std::cerr << "Inverted Calibration result (" << EETFname << " pose in "
                << ARTagTFname << " frame): \n";
      std::cerr << "Translation (x,y,z): "
                << resultAffineInv.translation().transpose() << std::endl;
      quaternionResult = Eigen::Quaternion<double>(resultAffineInv.rotation());
